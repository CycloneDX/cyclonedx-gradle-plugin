...
/**
 * This task mainly acts a container for the user configurations
 * (includeConfigs, projectType, schemaVersion, ...)
 * and orchestrating the calls between the core objects (SbomGraphProvider and
 * SbomBuilder)
 */
@CacheableTask
public abstract class CyclonedxDirectTask extends BaseCyclonedxTask {
    ...
    /**
     * Whether to include metadata resolution in the BOM. For example, license
     * information.
    ...
    /**
     * Whether to include the build environment dependencies (e.g. from buildscript)
     * in the BOM.
     * If not set, it defaults to false.
     *
     * @return true if build environment dependencies should be included, false
     *         otherwise
     */
    @Input
    ...
    /**
     * Executes the main logic of the plugin by loading the dependency graph
     * (SbomGraphProvider.get())
